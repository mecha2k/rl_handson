...
if __name__ == "__main__":
    ...
    proj_hist = lib.dqn_extra.distr_projection(
        src_hist, np.array([2], dtype=np.float32), np.array([False]), Vmin, Vmax, N_ATOMS, gamma=0.9
    )
    ...
    hist = np.histogram(
        data, normed=True, bins=np.arange(Vmin - DELTA_Z / 2, Vmax + DELTA_Z * 3 / 2, DELTA_Z)
    )
    ...
    proj_dist = lib.dqn_extra.distr_projection(
        np.array([src_hist]),
