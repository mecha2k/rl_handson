...
def batch_generator(
    buffer: ptan.experience.ExperienceReplayBuffer, initial: int, batch_size: int, steps: int
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--envs", type=int, default=3, help="Amount of environments to run in parallel"
    )
    ...
    for _ in range(args.envs):
        ...
        env = atari_wrappers.wrap_deepmind(
            env, pytorch_img=True, frame_stack=True, frame_stack_count=2
        )
    ...
    exp_source = ptan.experience.ExperienceSourceFirstLast(envs, agent, gamma=params.gamma)
    ...
    def episode_completed(trainer: Engine):
        print(
            ...
            % (
                ...
                trainer.state.metrics.get("fps", 0),
    ...
    logdir = (
        f"runs/{datetime.now().isoformat(timespec='minutes')}-{params.run_name}-{NAME}={args.envs}"
    )
