...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(comment="-pong-pg-loc-" + args.name)
    ...
    with common.RewardTracker(writer, stop_reward=18) as tracker:
        for step_idx, exps in enumerate(exp_source):
            ...
            for exp in reversed(exps):
                ...
                disc_reward *= GAMMA
                disc_reward += exp.reward
            baseline = baseline_buf.mean()
            batch_states.append(np.array(exps[0].state, copy=False))
            batch_actions.append(int(exps[0].action))

            if USE_MEAN_BASELINE:
                batch_scales.append(disc_reward - baseline)
            else:
