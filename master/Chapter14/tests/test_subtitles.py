...
class TestPhrases(TestCase):
    def test_split_phrase(self):
        ...
        phrase = libbots.data.Phrase(
            words=["-", "Wait", "a", "sec", ".", "-"],
            time_start=datetime.timedelta(0, 588, 204000),
...
class TestUtils(TestCase):
    def test_parse_time(self):
        self.assertEqual(
            subtitles.parse_time("00:00:33,074"), datetime.timedelta(seconds=33, milliseconds=74)
        )

    def test_remove_braced_words(self):
        self.assertEqual(subtitles.remove_braced_words(["a", "b", "c"]), ["a", "b", "c"])
        self.assertEqual(subtitles.remove_braced_words(["a", "[", "b", "]", "c"]), ["a", "c"])
        self.assertEqual(subtitles.remove_braced_words(["a", "[", "b", "c"]), ["a"])
        self.assertEqual(subtitles.remove_braced_words(["a", "]", "b", "c"]), ["a", "b", "c"])
        self.assertEqual(subtitles.remove_braced_words(["a", "(", "b", "]", "c"]), ["a", "c"])
        self.assertEqual(subtitles.remove_braced_words(["a", "(", "b", "c"]), ["a"])
        self.assertEqual(subtitles.remove_braced_words(["a", ")", "b", "c"]), ["a", "b", "c"])
