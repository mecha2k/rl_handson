...
if __name__ == "__main__":
    ...
    net = model.PhraseModel(
        emb_size=model.EMBEDDING_DIM, dict_size=len(emb_dict), hid_size=model.HIDDEN_STATE_SIZE
    ).to(device)
            ...
            for batch in data.iterate_batches(train_data, BATCH_SIZE):
                ...
                for idx, inp_idx in enumerate(input_batch):
                    ...
                    r_argmax, actions = net.decode_chain_argmax(
                        item_enc, beg_embedding, data.MAX_TOKENS, stop_at_token=end_token
                    )
                    ...
                    if not dial_shown:
                        ...
                        ref_words = [
                            utils.untokenize(data.decode_words(ref, rev_emb_dict))
                            for ref in ref_indices
                        ]
                        ref = " ~~|~~ ".join(ref_words)
                        log.info("Refer: %s", ref)
                        w = data.decode_words(actions, rev_emb_dict)
                        log.info("Argmax: %s, bleu=%.4f", utils.untokenize(w), argmax_bleu)
                    ...
                    for _ in range(args.samples):
                        ...
                        if not dial_shown:
                            w = data.decode_words(actions, rev_emb_dict)
                            log.info("Sample: %s, bleu=%.4f", utils.untokenize(w), sample_bleu)
