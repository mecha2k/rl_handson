...
class DQNAgent(ptan.agent.BaseAgent):
    def __init__(
        self, net: DQNModel, preprocessor: preproc.Preprocessor, epsilon: float = 0.0, device="cpu"
    ):
        self.net = net
    ...
    def __call__(self, states, agent_states=None):
        ...
        for state in states:
            ...
            if random.random() <= self.epsilon:
                ...
                obs_t = self._prepr.encode_sequences([state["obs"]]).to(self.device)
...
def calc_loss_dqn(batch, preprocessor, tgt_preprocessor, net, tgt_net, gamma, device="cpu"):
    observations, taken_commands, rewards, next_best_qs = unpack_batch(
...
class CommandModel(nn.Module):
    def __init__(
        ...
        self.rnn = nn.LSTM(
            input_size=embeddings.embedding_dim, hidden_size=obs_size, batch_first=True
        )
        self.out = nn.Linear(in_features=obs_size, out_features=dict_size)
...
class CmdAgent(ptan.agent.BaseAgent):
    def __init__(self, env, cmd: CommandModel, preprocessor: preproc.Preprocessor, device="cpu"):
        self.env = env
        self.cmd = cmd
...
def pretrain_loss(cmd: CommandModel, commands: List, observations_t: torch.Tensor):
    commands_batch = []
    target_batch = []
    min_length = None
    ...
    commands_batch = [c[: min_length - 1] for c in commands_batch]
    target_batch = [c[: min_length - 1] for c in target_batch]
...
def calc_loss_dqncmd(
    batch, preprocessor, cmd, cmd_encoder, tgt_cmd_encoder, net, tgt_net, gamma, env, device="cpu"
):
