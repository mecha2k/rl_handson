...
def remotes_url(port_ofs=0, hostname="localhost", count=8):
    hosts = [
        "%s:%d+%d" % (hostname, 5900 + ofs, 15900 + ofs)
        for ofs in range(port_ofs, port_ofs + count)
    ]
...
def save_obs(obs, file_name, action=None, action_step_pix=10, action_y_ofs=50, transpose=True):
    ...
    if action is not None:
        ...
        draw.ellipse(
            (x_ofs - half_step, y_ofs - half_step, x_ofs + half_step, y_ofs + half_step),
            (0, 0, 255, 128),
        )
    img.save(file_name)
...
class MiniWoBPeeker(vectorized.Wrapper):
    ...
    def _step(self, action_n):
        ...
        for idx, (obs, reward, done, action) in enumerate(
            zip(observation_n, reward_n, done_n, action_n)
        ):
