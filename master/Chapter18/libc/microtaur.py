...
class FourShortLegsRobot(robot_bases.MJCFBasedRobot):
    ...
    def show_joints(self):
        for j_idx in range(self._p.getNumJoints(self.objects[0])):
            ...
            print(
                "  name=%s, type=%d, qIndex=%d, uIndex=%d"
...
class FourShortLegsRobotParametrized(FourShortLegsRobot):
    ...
    def __init__(
        self,
        time_step: float,
        imu_observations: bool,
        params_vector: List[float],
        obs_bias: bool = False,
    ):
...
def register():
    gym_register(ENV_ID, entry_point="libc.microtaur:FourShortLegsEnv", max_episode_steps=1000)
