...
if __name__ == "__main__":
    ...
    if args.params == "noisynet":
        net = ppo.AtariNoisyNetsPPO(test_env.observation_space.shape, test_env.action_space.n).to(
            device
        )
    elif args.params == "distill":
        net = ppo.AtariDistillPPO(test_env.observation_space.shape, test_env.action_space.n).to(
            device
        )
        do_distill = True
    ...
    for _ in range(N_ENVS):
        ...
        if do_distill:
            env = common.NetworkDistillationRewardWrapper(
                env,
                reward_callable=get_distill_reward,
                reward_scale=params.distill_scale,
                sum_rewards=False,
            )
        envs.append(env)
    ...
    def test_network(engine):
        ...
        print(
            "Test done: got %.3f reward after %d steps, avg reward %.3f"
