...
class MountainCarBasePPO(nn.Module):
    def __init__(self, obs_size, n_actions, hid_size: int = 64):
        ...
        self.actor = nn.Sequential(
            nn.Linear(obs_size, hid_size), nn.ReLU(), nn.Linear(hid_size, n_actions),
        )

        self.critic = nn.Sequential(
            nn.Linear(obs_size, hid_size), nn.ReLU(), nn.Linear(hid_size, 1),
        )
...
class MountainCarNoisyNetsPPO(nn.Module):
    def __init__(self, obs_size, n_actions, hid_size: int = 128):
        ...
        self.actor = nn.Sequential(nn.Linear(obs_size, hid_size), nn.ReLU(), self.noisy_layers[0],)
...
def batch_generator(
    ...
    for (exp,) in exp_source:
        ...
        adv_t, ref_t = calc_adv_ref(
            trj_values_t.data.cpu().numpy(), trj_dones, trj_rewards, gamma, gae_lambda
        )
...
def batch_generator_distill(
    ...
    for (exp,) in exp_source:
        ...
        adv_t, _ = calc_adv_ref(
            (trj_values_ext_t + trj_values_int_t).data.cpu().numpy(),
            trj_dones,
            trj_rewards,
            gamma,
            gae_lambda,
        ...
        _, ref_ext_t = calc_adv_ref(
            trj_values_ext_t.data.cpu().numpy(), trj_dones, trj_rewards_ext, gamma, gae_lambda
        )
        ref_ext_t = ref_ext_t.to(device)

        _, ref_int_t = calc_adv_ref(
            trj_values_int_t.data.cpu().numpy(), trj_dones, trj_rewards_int, gamma, gae_lambda
        )
...
class AtariNoisyNetsPPO(nn.Module):
    ...
    def __init__(self, input_shape, n_actions):
        ...
        self.actor = nn.Sequential(self.noisy_layers[0], nn.ReLU(), self.noisy_layers[1],)
        self.critic = nn.Sequential(nn.Linear(conv_out_size, 256), nn.ReLU(), nn.Linear(256, 1))
