...
class PlayerBot:
    ...
    def _save_log(self, session, bot_score):
        self._update_leaderboard(
            bot_score, os.path.basename(session.model_file), session.player_id.split(":")[0]
        )
        data = {
            "ts": time.time(),
...
""",
    ...
    def text(self, bot, update):
        ...
        try:
            ...
            bot.send_message(
                chat_id=chat_id,
                text="I don't understand. In play mode you can give a number "
    ...
    def command_top(self, bot, update):
        ...
        l = "\n".join(res)
        bot.send_message(
            chat_id=update.message.chat_id, text="<pre>" + l + "</pre>", parse_mode="HTML"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--log", required=True, help="Log name to keep the games and leaderboard"
    )
    ...
    updater.dispatcher.add_handler(
        telegram.ext.CommandHandler("play", player_bot.command_play, pass_args=True)
    )
    updater.dispatcher.add_handler(
        telegram.ext.CommandHandler("refresh", player_bot.command_refresh)
    )
    updater.dispatcher.add_handler(
        telegram.ext.MessageHandler(telegram.ext.Filters.text, player_bot.text)
    )
