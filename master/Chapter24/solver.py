...
def gather_data(
    cube_env, net, max_seconds, max_steps, max_depth, samples_per_depth, batch_size, device
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--model", required=True, help="Model file to load, has to match env type"
    )
    parser.add_argument(
        "--max-time",
        type=int,
    ...
    parser.add_argument(
        "--max-steps",
        type=int,
        help="Limit amount of MCTS searches to be done. " "If specified, superseeds --max-time",
    )
    parser.add_argument(
        "--max-depth",
        type=int,
    ...
    parser.add_argument(
        "-b", "--batch", type=int, default=1, help="Batch size to use during the search, default=1"
    )
    ...
    group.add_argument(
        "-i",
        "--input",
        help="Text file with permutations to read cubes to solve, "
        "possibly produced by gen_cubes.py",
    )
    group.add_argument(
        "-p", "--perm", help="Permutation in form of actions list separated by comma"
    )
    group.add_argument(
        "-r", "--random", metavar="DEPTH", type=int, help="Generate random scramble of given depth"
    )
    ...
    if args.random is not None:
        ...
        with open(args.input, "rt", encoding="utf-8") as fd:
            for idx, l in enumerate(fd):
                ...
                count += 1
        log.info(
            "Solved %d out of %d cubes, which is %.2f%% success ratio",
            solved,
            count,
            100 * solved / count,
        )
