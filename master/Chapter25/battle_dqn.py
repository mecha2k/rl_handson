...
def test_model(
    net: model.DQNModel, device: torch.device, gw_config
    ...
    agent_a = ptan.agent.DQNAgent(
        net, ptan.actions.ArgmaxActionSelector(), device, preprocessor=preproc
    )
    agent_b = ptan.agent.DQNAgent(
        net, ptan.actions.ArgmaxActionSelector(), device, preprocessor=preproc
    )
...
def batch_generator(
    a_exp: ptan.experience.ExperienceSource,
...
if __name__ == "__main__":
    ...
    b_env = data.MAgentEnv(
        m_env, b_handle, reset_env_func=reset_env, is_slave=False, steps_limit=MAX_EPISODE
    )

    obs = data.MAgentEnv.handle_obs_space(m_env, a_handle)

    net = model.DQNModel(
        obs.spaces[0].shape, obs.spaces[1].shape, m_env.get_action_space(a_handle)[0]
    ).to(device)
    ...
    def test_network(engine):
        ...
        if best_test_reward is None:
            ...
            print("Best test reward updated %.3f <- %.3f, save model" % (best_test_reward, reward))
            best_test_reward = reward
            torch.save(net.state_dict(), os.path.join(saves_path, "best_%.3f.dat" % reward))

    engine.run(
        batch_generator(
            a_exp_source, b_exp_source, buffer, PARAMS.replay_initial, PARAMS.batch_size
        )
    )
