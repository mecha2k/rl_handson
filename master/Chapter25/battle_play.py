...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ma", "--model_a", required=True, help="Model file to load in the first group"
    )
    parser.add_argument(
        "-mb", "--model_b", required=True, help="Model file to load in the second group"
    )
    parser.add_argument("--map-size", type=int, default=16, help="Size of the map, default=64")
    parser.add_argument(
    ...
    v = (v[-1],) + v[:2]
    net_b = model.DQNModel(v, env.get_feature_space(b_handle), env.get_action_space(b_handle)[0])
    ...
    print(
        "Average reward: A %.3f, B %.3f"
        % (a_total_reward / args.count_a, b_total_reward / args.count_b)
    )
