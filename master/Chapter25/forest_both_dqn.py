...
def test_model(
    ...
    deer_agent = ptan.agent.DQNAgent(
        net_deer, ptan.actions.ArgmaxActionSelector(), device, preprocessor=preproc
    )
    tiger_agent = ptan.agent.DQNAgent(
        net_tiger, ptan.actions.ArgmaxActionSelector(), device, preprocessor=preproc
    )
...
def batches_generator(
    buffers: List[ptan.experience.ExperienceReplayBuffer], replay_initial: int, batch_size: int
):
    while True:
        for buf in buffers:
            buf.populate(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cuda", default=False, action="store_true", help="Enable CUDA computations"
    )
    ...
    net_tiger = model.DQNModel(
        tiger_obs.spaces[0].shape,
        tiger_obs.spaces[1].shape,
        m_env.get_action_space(tiger_handle)[0],
    ).to(device)
    ...
    def process_batches(engine, batches):
        ...
        for name, batch, opt, net, tgt_net in zip(
            ...
            loss_v = model.calc_loss_dqn(
                batch, net, tgt_net.target_model, preproc, gamma=PARAMS.gamma, device=device
            )
            loss_v.backward()
            opt.step()
    ...
    def test_network(engine):
        ...
        if best_test_reward_deer is None:
            ...
            print(
                "Best test deer reward updated %.3f <- %.3f, save model"
                % (best_test_reward_deer, deer_reward)
            )
            best_test_reward_deer = deer_reward
            torch.save(
                net_deer.state_dict(), os.path.join(saves_path, "deer_best_%.3f.dat" % deer_reward)
            )
        ...
        if best_test_reward_tiger is None:
            ...
            print(
                "Best test tiger reward updated %.3f <- %.3f, save model"
                % (best_test_reward_tiger, tiger_reward)
            )
            best_test_reward_tiger = tiger_reward
            torch.save(
                net_tiger.state_dict(),
                os.path.join(saves_path, "tiger_best_%.3f.dat" % tiger_reward),
            )

    engine.run(
        batches_generator([deer_buffer, tiger_buffer], PARAMS.replay_initial, PARAMS.batch_size)
    )
