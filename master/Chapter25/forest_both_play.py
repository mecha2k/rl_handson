...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-md", "--model_deer", required=True, help="Model file to load in deer agent"
    )
    parser.add_argument(
        "-mt", "--model_tiger", required=True, help="Model file to load in tiger agent"
    )
    parser.add_argument("--map-size", type=int, default=64, help="Size of the map, default=64")
    parser.add_argument(
    ...
    parser.add_argument("--tigers", type=int, default=10, help="Count of tigers, default=10")
    parser.add_argument("--deers", type=int, default=50, help="Count of deers, default=50")
    parser.add_argument(
        "--mode",
        default="forest",
    ...
    net_tiger = model.DQNModel(
        v, env.get_feature_space(tiger_handle), env.get_action_space(tiger_handle)[0]
    )
    ...
    net_deer = model.DQNModel(
        v, env.get_feature_space(deer_handle), env.get_action_space(deer_handle)[0]
    )
    ...
    print(
        "Average reward: tigers %.3f, deers %.3f"
        % (tiger_total_reward / args.tigers, deer_total_reward / args.deers)
    )
